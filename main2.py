...
class App:
    def __init__(self, root):
        ...
        self.zant_button = tk.Button(root, text=" 暂停 ", command=self.zant, bg="#808080")
        self.zant_button.grid(row=0, column=3, padx=3, pady=3)

        self.position_button = tk.Button(root, text=" 连接vz ", command=self.start_tcp_server,bg="#808080")
        self.position_button.grid(row=0, column=4, padx=3, pady=3)

        # 创建一个 Frame 用于放置动态添加的行
        self.frame = tk.Frame(root)
        self.frame.grid(row=1, column=0, columnspan=5, sticky="nsew")
        self.image = PhotoImage(file="55.png")
        self.label = tk.Label(root, image=self.image)
        self.label.grid(row=2, column=0, columnspan=5, sticky="nsew")
